...
if __name__ == "__main__":
    ...
    y = dcb.getDiscretization(x=[0, h], d=[0.01, 1])
    ...
    for idx in range(0, len(x)):
        for idy in range(0, len(y)):
            ...
            string, stringBC, num, datx, daty = dcb.createPlate(
                x=x[idx], y=y[idy], dfunx=dfunx[idx], dfuny=dfuny[idy], k=k, numIn=num
            )
    ...
    # top plates
    print(
        "Number of Nodes: ", num, "horizon 1 4dx: ", 4 * dx, "horizon 2 4dx: ", 4 * da
    )
    run = False
    if run:
        num = 0
        ...
        # adhesive
        string, stringBC, num, datx, daty = dcb.createPlate(
            x=[0, lp], y=[h, h + ta], dfunx=[dx, da], dfuny=[da, da], k=2, numIn=num
        )
        writeString += string
        writeStringLC += stringBC
        plt.plot(datx, daty, "*")
