...
class KalthoffWinkler(object):
    def __init__(self, xend = 100, yend = 100, zend = 0.003, dx=[0.001,0.001,0.001], 
    filename = 'Kalthoff-Winkler', TwoD = False, rot = 'False', angle = [0,0], 
    material = '', damage = '', block = '', bc = '', bf = '', compute = '', output = '', solver = '', username = '', maxNodes = 10000000, ignoreMesh = False):
        '''
        ...
        self.angle = [0,0]
        if damage=='':
            damageDict = Damage(id=1, Name='PMMADamage', damageModel='Critical Stretch', criticalStretch=10,criticalEnergy=0.0022170, interblockdamageEnergy=0.01, planeStress=True, onlyTension=False, detachedNodesCheck=True, thickness=10, hourglassCoefficient=1.0, stabilizatonType='Global Stiffness')
        ...
        if output=='':
            outputDict1 = Output(id=1, Name='Output1', Displacement=True, Force=True, Damage=True, Velocity=True, Partial_Stress=False, External_Force=False, External_Displacement=False, Number_Of_Neighbors=True, Frequency=10, InitStep=0)
        ...
        if material=='':
            ...
            for material in matNameList:
                matDict = Material(id=i+1, Name=material, MatType='Elastic Bond Based', density=8.e-6, bulkModulus=1.2666666666666667e+5, shearModulus=None, youngsModulus=None, poissonsRatio=None, tensionSeparation=False, nonLinear=True, planeStress=True, materialSymmetry='Isotropic', stabilizatonType='Global Stiffness', thickness=10.0, hourglassCoefficient=1.0, actualHorizon=None, yieldStress=None, Parameter=[] ,Properties=[])
                i+=1
        ...
        if bf=='': 
            bf1 = BondFilters(id=1, Name='bf_1', type='Rectangular_Plane', normalX=0.0, normalY=1.0, normalZ=0.0, lowerLeftCornerX=-0.5, lowerLeftCornerY=25.0, lowerLeftCornerZ=-0.5, bottomUnitVectorX=1.0, bottomUnitVectorY=0.0, bottomUnitVectorZ=0.0, bottomLength=50.5, sideLength=1.0, centerX=0.0, centerY=1.0, centerZ=0.0, radius=1.0, show=True)
            bf2 = BondFilters(id=1, Name='bf_2', type='Rectangular_Plane', normalX=0.0, normalY=1.0, normalZ=0.0, lowerLeftCornerX=-0.5, lowerLeftCornerY=-25.0, lowerLeftCornerZ=-0.5, bottomUnitVectorX=1.0, bottomUnitVectorY=0.0, bottomUnitVectorZ=0.0, bottomLength=50.5, sideLength=1.0, centerX=0.0, centerY=1.0, centerZ=0.0, radius=1.0, show=True)
            self.bondfilters = [bf1,bf2]
        else:
            self.bondfilters = bf

        if bc=='': 
            bc1 = BoundaryConditions(id=1, Name='BC_1', NodeSets=None, boundarytype='Prescribed Displacement', blockId=2, coordinate='x', value='0*t')
            bc2 = BoundaryConditions(id=2, Name='BC_2', NodeSets=None, boundarytype='Prescribed Displacement', blockId=3, coordinate='x', value='0*t')
            bc3 = BoundaryConditions(id=3, Name='BC_3', NodeSets=None, boundarytype='Prescribed Displacement', blockId=4, coordinate='x', value='10*t')
    ...
    def createBoundaryConditionBlock(self,x,y,k):
        k = np.where(np.logical_and(x > self.xend-self.dx[0]*3,np.logical_and(y < 90 + self.dx[0]*3, y > 90 - self.dx[0]*3)), 2, k)
        k = np.where(np.logical_and(x > self.xend-self.dx[0]*3,np.logical_and(y < -90 + self.dx[0]*3, y > -90 - self.dx[0]*3)), 3, k)
        return k

    def createLoadIntroNode(self,x,y, k):
        k = np.where(np.logical_and(x < self.dx[0]*3,np.logical_and(y < 25 , y > -25)), 4, k)
        return k
