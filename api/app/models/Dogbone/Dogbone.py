    ...
    def createModel(self):
        ...
        if self.ignoreMesh and self.blockDef != "":
            ...
            if self.structured:
                ...
                numRows = int((nn - 1) / 2)
                fh2 = (2 * self.dx[1] * (numRows) + self.h2 - self.h1) / (
                    self.dx[1] * (numRows)
                )
                x = []
                y = []
                ...
                for zval in z0:
                    for i in range(0, numRows):
                        ...
                        topSurf, bottomSurf = geo.createBoundaryCurve(
                            h=h1 / 2,
                            l1=l1,
                            R=R,
                            l2=l2,
                        ...
                        blockDef = np.array(
                            [0, bc, l1, l1 + 2 * dl + l2, self.xend - bc]
                        )
                        ...
                        for xval in x0:
                            for idx, val in enumerate(blockDef):
                                if geo.checkValGreater(xval, val):
                                    matNum = idx + 1
                            k.append(matNum)
                        for xval in x0:
                            for idx, val in enumerate(blockDef):
                ...
                topSurf, bottomSurf = geo.createBoundaryCurveOld(
                    h=self.h1 / 2, l1=l1, R=R, l2=l2, alphaMax=alpha, dl=dl, dh=dh
                )
                ...
                for xval in x0:
                    for yval in y0:
                        for zval in z0:
                            if geo.checkValGreater(
                                yval, bottomSurf(xval)
                            ) and geo.checkValLower(yval, topSurf(xval)):
                                for idx, val in enumerate(blockDef):
                                    if geo.checkValGreater(xval, val):
                                        matNum = idx + 1
                                x.append(xval)
