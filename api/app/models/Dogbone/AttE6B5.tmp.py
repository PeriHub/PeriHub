...
if __name__ == '__main__':
    ...
    topSurf, bottomSurf = c.createBoundaryCurve(h = h1, l1 = l1, R = R, l2 = l2, alphaMax = alpha, dl = dl, dh = dh)
    ...
    for xval in x:
        for yval in y:
            for zval in z:
                if c.checkValGreater(yval,bottomSurf(xval)) and c.checkValLower(yval,topSurf(xval)):
                    num += 1
                    for idx,val in enumerate(blockDef): 
                        if c.checkValGreater(xval,val): matNum = idx + 1
    ...
    for zval in z: 
        string += str(l1+dl) + ' ' + str(0.5*(h1-h2)) + ' ' +  str(zval)  + ' ' + str(3) + ' ' + str(vol) + '\n' 
    xList.append(l1+dl);                yList.append(0.5*(h1-h2))
    ...
    fid.close()
    print('numer of Nodes',num,'horizon 4dx', 4.01*dx,' numer of BC nodes', bccount)
