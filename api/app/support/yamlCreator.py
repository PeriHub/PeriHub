    ...
    def solver(self):
        ...
        if(self.solverDict['solvertype']=='Verlet'):
            ...
            if('adaptivetimeStepping' in self.solverDict):
                ...
                string += '            Stable Bond Difference: '+ str(self.solverDict['adapt']['stableBondDifference']) +'\n'
        elif(self.solverDict['solvertype']=='NOXQuasiStatic'):
            string += '        Peridigm Preconditioner: '+ str(self.solverDict['peridgimPreconditioner']) +'\n'
            string += '        NOXQuasiStatic:\n'
            string += '            Nonlinear Solver: '+ str(self.solverDict['nonlinearSolver']) +'\n'
            string += '            Number of Load Steps: '+ str(self.solverDict['numberofLoadSteps']) +'\n'
            string += '            Max Solver Iterations: '+ str(self.solverDict['maxSolverIterations']) +'\n'
            string += '            Relative Tolerance: '+ str(float(self.solverDict['relativeTolerance'])) +'\n'
            string += '            Max Age Of Prec: '+ str(self.solverDict['maxAgeOfPrec']) +'\n'
            string += '            Direction:\n'
            string += '                 Method: '+ str(self.solverDict['directionMethod']) +'\n'
            if(self.solverDict['directionMethod']=='Newton'):
                string += '                 Newton:\n'
                string += '                      Linear Solver:\n'
                string += '                           Jacobian Operator: '+ str(self.solverDict['newton']['jacobianOperator']) +'\n'
                string += '                           Preconditioner: '+ str(self.solverDict['newton']['preconditioner']) +'\n'
            string += '            Line Search:\n'
            string += '                 Method: '+ str(self.solverDict['lineSearchMethod']) +'\n'
            if(self.solverDict['verletSwitch']):
                string += '            Switch to Verlet:\n'
                string += '                 Safety Factor: '+ str(float(self.solverDict['verlet']['safetyFactor'])) +'\n'
                string += '                 Numerical Damping: '+ str(float(self.solverDict['verlet']['numericalDamping'])) +'\n'
