    ...
    def getUserName(request, dev):
        if dev:
            ...
            decodedToken = jwt.decode(
                encodedToken.split(" ")[1], options={"verify_signature": False}
            )
            ...
            if os.path.getmtime(folderPath) < now - days * 86400:
                ...
                if recursive:
                    for subfoldername in os.listdir(folderPath):
                        if (
                            os.path.getmtime(os.path.join(folderPath, subfoldername))
                            < now - days * 86400
                        ):
                ...
                if recursive:
                    for subfoldername in sftp.listdir(folderPath):
                        if (
                            ...
                            fileHandler.removeAllFolderSftp(
                                sftp, os.path.join(folderPath, subfoldername), True
                            )
    ...
    def removeAllFolderSftp(sftp, remotepath, recursive):
        ...
        for filename in sftp.listdir(remotepath):
            for subfilename in sftp.listdir(os.path.join(remotepath, filename)):
                if recursive:
                    for subSubfilename in sftp.listdir(
