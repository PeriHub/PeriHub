        ...
        if use_cell_data:
            ...
            for block_id in range(0,len(block_data)):
                ...
                if block_id in cell_data[variable][0] and max(cell_data[variable][-1][block_id][0]) > 0:
                    cell_value = np.concatenate([cell_value, cell_data[variable][-1][block_id][0]])
                else:
                    cell_value = np.concatenate([cell_value, np.full_like(np_first_points_x,0)])

            triang = mtri.Triangulation(np_points_x, np_points_y)
            mask = VideoExport.long_edges(np_points_x, np_points_y, triang.triangles, max_edge_distance)
            triang.set_mask(mask)
            
            # plt.triplot(triang)
            tcf = ax.tricontourf(triang, cell_value, levels=100, cmap=cm.jet, vmin=v_min, vmax=v_max)
            ...
            def animate(i, triang):
                ...
                for block_id in range(0,len(block_data)):
                    ...
                    if block_id in cell_data[variable][0] and max(cell_data[variable][0][block_id][i]) > 0:
                        cell_value = np.concatenate([cell_value, cell_data[variable][0][block_id][i]])
                    else:
                ...
                if apply_displacements:   
                    ...
                    mask = VideoExport.long_edges(np_points_all_x, np_points_all_y, triang.triangles, max_edge_distance)
                    triang.set_mask(mask)
            ...
            mask = VideoExport.long_edges(points[:,0], points[:,1], triang.triangles, max_edge_distance)
            triang.set_mask(mask)
            # plt.triplot(triang)

            tcf = ax.tricontourf(triang, point_data[variable][0][axis_id][:], levels=100, cmap=cm.jet, vmin=v_min, vmax=v_max)
            ...
            def animate(i, triang):
                ...
                tcf = ax.tricontourf(triang, point_data[variable][i][axis_id][:], levels=100, cmap=cm.jet, vmin=v_min, vmax=v_max)
                cax.cla()
                fig.colorbar(tcf, cax=cax)
                tx.set_text('Frame {0}'.format(i))
