...
class VerificationModels(object):
    def __init__(self, TwoD=True):
        ...
        self.materialDict[0]["Parameter"] = mat.stiffnessMatrix(
            type="isotropic", matParam=params
        )
        params = [270000, 2.1e11, 0.3, 0, 0]
        self.materialDict[1]["Parameter"] = mat.stiffnessMatrix(
            type="isotropic", matParam=params
        )
        self.angle = [0, 0]
        params = [
        ...
        self.materialDict[2]["Parameter"] = mat.stiffnessMatrix(
    ...
    def createVerificationModels(self):
        ...
        self.callModelbuilder(
            TwoD=True, angle=[90, 90], filename="twoLayerAniso090Tension2D"
        )
        self.bcDict.append(
            {
                "Name": "BC_7",
        ...
        self.callModelbuilder(
            TwoD=False, angle=[0, 90], filename="twoLayerAniso090Tension3D"
        )
        del self.bcDict[-1]
        self.callModelbuilder(
            TwoD=True, angle=[30, -30], filename="twoLayerAniso30m30Tension2D"
        )
        self.bcDict.append(
            {
                "Name": "BC_7",
        ...
        self.callModelbuilder(
            TwoD=False, angle=[30, -30], filename="twoLayerAniso30m30Tension3D"
        )
        self.bcDict[0]["coordinate"] = "y"
        self.bcDict[1]["coordinate"] = "y"
        # fehlt noch was
        del self.bcDict[-1]
        self.callModelbuilder(
            TwoD=True, angle=[0, 90], filename="twoLayerAniso090Bending2D"
        )
        self.bcDict.append(
            {
                "Name": "BC_7",
        ...
        self.callModelbuilder(
            TwoD=False, angle=[9, 90], filename="twoLayerAniso090Bending3D"
        )
        del self.bcDict[-1]
        self.callModelbuilder(
            TwoD=True, angle=[30, -30], filename="twoLayerAniso30m30Bending2D"
        )
        self.bcDict.append(
            {
                "Name": "BC_7",
        ...
        self.callModelbuilder(
            TwoD=False, angle=[30, -30], filename="twoLayerAniso30m30Bending3D"
        )
    ...
    def createModel(self, TwoD, xend, yend, zend, dx, angle=[0, 0]):
        ...
        if TwoD:
            x, y, z = geo.createPoints(
                coor=[-3 * dx[0], xend + 3 * dx[0], 0, yend, 0, 0], dx=[dx[0], dx[1], 1]
            )
        else:
            x, y, z = geo.createPoints(
                coor=[-3 * dx[0], xend + 3 * dx[0], 0, yend, 0, zend],
                dx=[dx[0], dx[1], dx[2]],
            )
        ...
        if self.damageDict == "":
            ...
            for idx in range(0, blockLen):
                blockDef[idx] = {
                    ...
                    "material": self.matBlock[idx],
                    "horizon": self.scal * max([self.dx[0], self.dx[1]]),
                    "damageModel": self.damageDict[idx],
                }
        # 3d tbd
        return blockDef
    ...
    def writeFILE(self, writer, model):
        ...
        if self.blockDef == "":
            ...
            for idx in range(0, len(self.blockDef)):
                self.blockDef[idx]["horizon"] = self.scal * max(
                    [self.dx[0], self.dx[1]]
                )
