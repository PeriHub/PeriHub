...
<script lang="ts">
  ...
  // Optional backend fields (e.g. solver.static.showSolverIteration,
  // block.show) are typed `boolean | null` by the generated client, so a
  // bindable prop here must accept `null`. Svelte disallows binding
  // `undefined` into a bindable prop that *has* a fallback, so this prop
  // has none; null/undefined are normalized to boolean below.
  interface Props {
  ...
  // internally, so passing an upstream `null`/`undefined` straight through via
