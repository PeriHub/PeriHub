...
<script lang="ts">
  ...
  function addAdditiveModel() {
    ...
    const newItem = len > 0 ? structuredClone($state.snapshot(list[len - 1])) : ({} as (typeof list)[number]);
    ...
    <div class="space-y-3 border-t border-border pt-3">
      {#each additive.additiveModels ?? [] as model, index (model.additiveModelId ?? index)}
        <div class="flex flex-wrap items-end gap-3 border-b border-border pb-3">
          ...
          <Button variant="ghost" size="icon" onclick={() => removeAdditiveModel(index)} title="Remove additive model">
