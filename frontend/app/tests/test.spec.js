...
test('Tools Page', async ({ page }) => {
  ...
  await page
    ...
    .getByLabel("Poisson's Ratio(v)")
    .fill('0.33');
  await expect(page.getByRole('textbox', { name: 'Bulk Modulus (K)' }).nth(1)).toHaveValue(
    '3.7254901960784314e+3',
  );
  await expect(page.getByRole('textbox', { name: 'Shear Modulus (G)' }).nth(1)).toHaveValue(
    '1.4285714285714284e+3',
  );
  await expect(page.getByRole('textbox', { name: 'P-wave modulus (M)' }).nth(1)).toHaveValue(
    '5.6302521008403355e+3',
  );
  await expect(page.getByRole('textbox', { name: "Lamé's first parameter(m)" }).nth(1)).toHaveValue(
    '2.773109243697479e+3',
  );
});
