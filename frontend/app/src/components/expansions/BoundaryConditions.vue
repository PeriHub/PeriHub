    ...
    <div>
        <q-list v-for="nodeSet in boundaryConditions.nodeSets" :key="nodeSet.nodeSetId" style="padding: 0px">
            <div class="row my-row">
                <q-input class="my-input" v-model="nodeSet.file" :rules="[rules.required, rules.name]" label="Nodeset"
        ...
        <q-list v-for="boundaryCondition, index in boundaryConditions.conditions" :key="boundaryCondition.conditionsId"
            ...
            <div class="row my-row">
                <q-select v-show="!model.ownModel" class="my-input" :options="blocks" option-label="blocksId"
                    option-value="blocksId" emit-value v-model="boundaryCondition.blockId" :label="boundaryKeys.blockId"
                    clearable standout dense></q-select>
