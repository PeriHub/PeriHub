  ...
  <div>
    ...
    <q-list v-for="compute, index in computes" :key="compute.computesId" style="padding: 0px">
      <div class="row my-row">
        ...
        <q-select class="my-select" :options="variables" v-model="compute.variab    le" :label="computeKeys.variable"
          standout dense></q-select>
      </div>
      <div class="row my-row" v-show="compute.computeClass ==     'Block_Data'">
        <q-select class="my-select" :options="calculati    onType" v-model="compute.ca    lculationType"
      ...
      <div class="row my-row" v-show="comp    ute.computeClass == 'Neares    t_Point_Data'">
        ...
        <q-input class="my-input" v-model="compute.y" :rules="[rules.required, rules.    name]" :label="computeKey    s.y"
          clearable standout dense></q-input>
        <q-input class="my-input" v-model="compute.z" :rules="    [rules.required    , rules.name]" :label="computeK    eys.z"
          clearable standout dense></q-input>
      </div>
    ...
    <q-list v-for="output, index in outputs" :key="output.outputsId" style="padding: 0px">
      <div
        v-bind:style="(output.output    sId % 2 == 0) ? 'background-c    olor: rgba(190, 190, 190, 0.1);' : '    background-color: rgba(255, 255,     255, 0.0);'">
        <h4 class="my-title">Output {{ output.o    utputsId }}</h4>
        <div class="row my-row">
        ...
        <div class="row my-row">
          <q-select class="my-input" v-model="output.selec    tedOutputs" use-input use-chips multiple input-debounce="0"
            :options="filterOptions" @filter="filterFn" style="width: 250px; margin-bottom:20px;" standout
            dense></q-select>
          <q-select class="my-input" v-model="output.se    lectedFileType" :options="fileTypes" input-debounce="0"
          ...
          <q-toggle class="my-toggle" v-model="output.useOutputFrequency" label="Use Output Frequency" standout
            dense></q-toggle>
          <q-input v-if="    output.useOutputFrequen    cy" class="my-input" v-model="output.Frequency" :rules="    [rules.int]"
            label="Output Frequency" standout dense></q-input>
          <q-input v-if="!output.useOutputF    requency" class="my-input" v-model="output.numbe    rOfOutputSteps"
