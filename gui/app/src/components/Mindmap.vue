  ...
  <div>
    ...
    <v-card v-show="showDetails" class="drawer-container">
      <div class="text-align-left margin-top-10">
        <span class="span-title">Selected Node</span
        ><span class="span-id-margin">nodeId: {{ nodeId }}</span>
        <span>nodeName: {{ nodeName }}</span>
      </div>
      <div>
        <v-text-field
          label="Insert/Edit Node"
          outlined
          maxlength="50"
          dense
          v-model="newNodeName"
...
export default {
  ...
  methods: {
    ...
    clickNode(d) {
      ...
      this.currentNode = d.currentTarget.__data__;
      this.nodeId = this.currentNode.data.value;
      this.nodeName = this.currentNode.data.name;
      if (this.currentNode.children) {
        this.$set(this.currentNode, "_children", this.currentNode.children);
        this.currentNode.children = null;
      } else {
        this.$set(this.currentNode, "children", this.currentNode._children);
